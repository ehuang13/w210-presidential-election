...
#                    'twt/candidate/2020/biden/review.txt' news/candidates/cnn/biden
...
def data_init(path, DSC):
    ...
    for process in ['train/review.txt', 'train/{}_review.txt'.format(DSC), 
                    ...
                    'news/immigration/fox-news/trump/review.txt',
                    'news/immigration/fox-news/biden/review.txt'
                    ]:
...
def read_data(fname, source_word2idx, max_sent_length, target_maxlen, mode=None):
    ...
    try:
        dates = open(fname + r'dates.txt', 'r', encoding='utf-8').readlines()
    except:
    ...
    for index, _ in enumerate(review):
        ...
        if dates != None:
            filtered_dates.append(dates[index])
        
        
    if len(filtered_dates) > 0:
        dts = open(fname + r'datesf.txt', "a")
        for mp in filtered_dates:
            dts.write(str(mp))
            dts.write("\n")
        dts.close()
